"""Tests for the `process` audit cleanup (domain-neutralization + dedup).
...
- the shared `_process_one_document` helper (success + failure record shapes)
...
# ── _process_one_document (shared per-document core) ─────────────────────────


def test_process_one_document_success_shape(tmp_path):
...
def test_process_one_document_failure_shape(tmp_path):
    doc = tmp_path / "sample.txt"
    doc.write_text("hello world", encoding="utf-8")
