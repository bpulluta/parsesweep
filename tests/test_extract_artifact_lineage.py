...
def test_validate_runtime_cli_json_reports_compilation_output_error(tmp_path) -> None:
    pack_path = tmp_path / 'broken/pack.yaml'
    profile_path = tmp_path / 'profiles/default.profile.json'
    schema_path = tmp_path / 'schemas/personal/broken.json'
...
def test_init_domain_pack_cli_creates_pack_and_validates_runtime(tmp_path) -> None:
    ...
    assert 'compilation:' in pack_text
...
def test_init_domain_pack_cli_scaffolds_flat_qaqc_and_compilation(tmp_path) -> None:
    runner = CliRunner()
...
def test_init_domain_pack_cli_can_scaffold_workspace_folders(tmp_path, monkeypatch) -> None:
    ...
    expected_paths = [
        ...
        REPO_ROOT / 'extracted/workspace_domain',
        REPO_ROOT / 'compiled/workspace_domain',
    ]
...
def test_init_domain_pack_cli_can_scaffold_config_files(tmp_path) -> None:
    ...
    assert 'run.yaml: Starter runtime config for discover/extract/compile commands' in readme_path.read_text(encoding='utf-8')
    ...
    assert 'extraction:' in run_config_path.read_text(encoding='utf-8')
    assert 'compilation:' in run_config_path.read_text(encoding='utf-8')
    ...
    assert 'extracted/config_domain/qa_qc' not in readme_path.read_text(encoding='utf-8')
    ...
    assert next_steps[3]['command'] == (
        'pixi run psweep compile extracted/config_domain '
        '--schema schemas/personal/electricity_tariff_schema.json'
    )
...
def test_init_domain_pack_cli_scaffolds_domain_aware_config_content(tmp_path) -> None:
    ...
    assert 'extracted/geo_domain/qa_qc' not in readme_path.read_text(encoding='utf-8')
    assert '--qaqc-lane qualitative' not in readme_path.read_text(encoding='utf-8')
...
def test_init_domain_pack_cli_text_output_includes_next_steps(tmp_path) -> None:
    ...
    assert 'pixi run psweep check-runtime --pack' in result.output
    assert 'pixi run psweep extract documents/guided_domain/' in result.output
    ...
    assert 'pixi run psweep compile extracted/guided_domain --schema schemas/personal/geothermal_ordinance_schema.json' in result.output
    assert 'Optional multi-model QA/QC run' not in result.output
    assert '--enable-qa-qc' not in result.output
    assert 'extracted/guided_domain/qa_qc' not in result.output
...
def test_compile_cli_uses_runtime_pack_default_format_for_geothermal(tmp_path) -> None:
    extracted_dir = tmp_path / 'extracted/geothermal'
    output_dir = tmp_path / 'compiled/geothermal'
    _write_json(
