...
# AQTOOLKIT_DIR = VALIDATION_DIR / "llamaextract"
...
PERMIT_FIELDS = [
    ...
    ("State Facility ID", "permitDetails.stateFacilityID"),
    (
        "Construction Notification Required",
        "permitDetails.initialConstructionCommencedNotificationRequired",
    ),
    (
        "Construction Notification Window (Days)",
        "permitDetails.constructionCommencedNotificationWindowDays",
    ),
    (
        "Startup Notification Required",
        "permitDetails.initialStartupNotificationRequired",
    ),
    (
        "Startup Notification Window (Days)",
        "permitDetails.startupNotificationWindowDays",
    ),
...
GENERATOR_FIELDS = [
    ...
    ("Original Permit Date", "originalPermitDate"),
    ("Equipment Facility ID", "equipmentFacilityID"),
    ("Make", "make"),
    ("Model", "model"),
    ("Rated Capacity (BHP)", "ratedCapacityBHP"),
    ("Rated Capacity (HP)", "ratedCapacityHP"),
    ("Rated Capacity (kW)", "ratedCapacityKW"),
    ("Rated Capacity (MMBtu/hr)", "ratedCapacityMMBtuPerHr"),
    ...
    ("Fuel Sulfur Content (%)", "fuelSulfurContentPct"),
    ("Fuel Certification Required", "fuelCertificationRequired"),
    ...
    (
        "Operating Hours Per-Unit Rolling Window",
    ...
    (
        "Operating Hours Combined Rolling Window",
...
def find_permit_file(
    base_dir: Path, permit_number: str, state: str
) -> Optional[Path]:
...
def load_ground_truth_data(existing_file: Path) -> dict:
    ...
    try:
        ...
        for sheet_name in xls.sheet_names:
            ...
            if (
                "Extraction Parameter" in df.columns
                and "Ground Truth" in df.columns
            ):
            ...
            print(
                f"  Loaded {len(field_mapping)} ground truth values from {sheet_name}"
            )
...
def match_ground_truth(field_name: str, gt_mapping: dict) -> Any:
    ...
    for key, value in gt_mapping.items():
        ...
        if (
            normalized_field in normalized_key
            or normalized_key in normalized_field
        ):
            ...
            if (
                len(field_tokens & key_tokens)
                >= min(len(field_tokens), len(key_tokens)) - 1
            ):
                return value
...
def create_permit_sheet(
    permit_number: str,
    state: str,
    aqt_data: Optional[dict],
    llama_data: Optional[dict],
    dtree_data: Optional[dict],
    gt_mapping: dict,
) -> pd.DataFrame:
    ...
    rows.append(
        {
            ...
            "Decision Tree": "",
            "Notes": "",
            "_section": "permit_section",
        }
    )

    # Permit fields - NO EMPTY ROWS
    for display_name, json_path in PERMIT_FIELDS:
        aqt_val = get_nested_value(
            aqt_data.get("data", {}) if aqt_data else {}, json_path
        )
        llama_val = get_nested_value(
            llama_data.get("data", {}) if llama_data else {}, json_path
        )
        dtree_val = get_nested_value(
            dtree_data.get("data", {}) if dtree_data else {}, json_path
        )
    ...
    aqt_gens = (
        aqt_data.get("data", {}).get("generatorSets", []) if aqt_data else []
    )
    llama_gens = (
        llama_data.get("data", {}).get("generatorSets", [])
        if llama_data
        else []
    )
    dtree_gens = (
        dtree_data.get("data", {}).get("generatorSets", [])
        if dtree_data
        else []
    )
    ...
    for i in range(max_gens):
        ...
        rows.append(
            {
                "Field Name": f"GENERATOR SET {i + 1}",
                "Ground Truth": "",
                "AQ Toolkit": "",
                "LlamaExtract": "",
                "Decision Tree": "",
                "Notes": "",
                "_section": "generator_section",
            }
        )
        ...
        for display_name, field_key in GENERATOR_FIELDS:
            ...
            llama_val = (
                get_nested_value(llama_gen, field_key) if llama_gen else None
            )
            dtree_val = (
                get_nested_value(dtree_gen, field_key) if dtree_gen else None
            )
...
def apply_styling(worksheet, df):
    ...
    permit_section_fill = PatternFill(
        start_color="4472C4", end_color="4472C4", fill_type="solid"
    )
    generator_section_fill = PatternFill(
        start_color="70AD47", end_color="70AD47", fill_type="solid"
    )
    section_font = Font(bold=True, color="FFFFFF", size=11)

    # Field name column
    field_name_fill = PatternFill(
    ...
    for row_num in range(2, len(df) + 2):
        ...
        for col_num in range(1, 7):
            ...
            if section_type == "header":
                ...
                section_type == "permit_section"
                or section_type == "generator_section"
            ):
                # Section headers
...
def main():
    ...
    for state_dir in AQTOOLKIT_DIR.iterdir():
        if state_dir.is_dir() and state_dir.name not in {
            ".DS_Store",
            ".gitkeep",
        }:
            for json_file in state_dir.glob("*.json"):
                if json_file.stem != "validation_consolidated":
                    ...
                    if len(sheet_name) > 31:
                        ...
                        sheet_name = (
                            f"{state_dir.name}_{clean_permit[:max_permit_len]}"
                        )
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        for permit_info in permits:
            ...
            llama_file = find_permit_file(
                LLAMAEXTRACT_DIR, permit_number, state
            )
            dtree_file = find_permit_file(
                DECISIONTREE_DIR, permit_number, state
            )
            ...
            for gt_key in possible_keys:
                if gt_key in ground_truth:
                    ...
                    print(
                        f"  Found ground truth with key: {gt_key} ({len(gt_mapping)} values)"
                    )
                    break

            # Create sheet
            df = create_permit_sheet(
                permit_number,
                state,
                aqt_data,
                llama_data,
    ...
    print(
        "  - Color coded: Headers (dark blue), Sections (blue), Fields (gray), Ground Truth (yellow)"
    )
