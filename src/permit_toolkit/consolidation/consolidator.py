...
def calculate_missing_capacity(
    ...
    if power_hp is not None:
        ...
        if mmbtu_val is None:
            ...
            result["rated_capacity_mmbtu_per_hr"] = (
                mmbtu_hr  # Preserve original value
            )
...
class PermitConsolidator:
    ...
    def flatten_permit(
        ...
        base_info = {
            ...
            "permit_expiration_date": permit_details.get(
            ...
            "initial_construction_commenced_notification_required": permit_details.get(
                "initialConstructionCommencedNotificationRequired"
            ),
            "construction_commenced_notification_window_days": permit_details.get(
                "constructionCommencedNotificationWindowDays"
            ),
            "initial_startup_notification_required": permit_details.get(
                "initialStartupNotificationRequired"
            ),
            "startup_notification_window_days": permit_details.get(
                "startupNotificationWindowDays"
            ),
            "permit_copy_onsite_required": permit_details.get(
        ...
        for gen in generator_sets:
            ...
            if isinstance(allowed_modes, list):
                allowed_modes_str = (
                    "; ".join(allowed_modes) if allowed_modes else None
                )
            ...
            record.update(
                {
                    ...
                    "included_in_permit_project": gen.get(
                    ...
                    "rated_capacity_bhp": capacity_values[
                        "rated_capacity_bhp"
                    ],
                    "rated_capacity_hp": capacity_values["rated_capacity_hp"],
                    "rated_capacity_kw": capacity_values["rated_capacity_kw"],
                    "rated_capacity_mmbtu_per_hr": capacity_values[
                        "rated_capacity_mmbtu_per_hr"
                    ],
                    ...
                    "calculated_mmbtu_per_hr": capacity_values[
                        "calculated_mmbtu_per_hr"
                    ],
                    ...
                    "fuel_certification_required": gen.get(
                        "fuelCertificationRequired"
                    ),
                    "fuel_change_permit_trigger": gen.get(
                        "fuelChangePermitTrigger"
                    ),
                    # Fuel - throughput per unit
                    "fuel_throughput_per_unit_limit": gen.get(
                        "fuelThroughputPerUnitLimit"
                    ),
                    "fuel_throughput_per_unit_scope": gen.get(
                        "fuelThroughputPerUnitScope"
                    ),
                    "fuel_throughput_per_unit_group_ref": gen.get(
                        "fuelThroughputPerUnitGroupRef"
                    ),
                    # Fuel - throughput combined
                    "fuel_throughput_combined_limit": gen.get(
                        "fuelThroughputCombinedLimit"
                    ),
                    "fuel_throughput_combined_group_ref": gen.get(
                        "fuelThroughputCombinedGroupRef"
                    ),
                    # Control technology
                    "control_technology": gen.get("controlTechnology"),
                    # Operating hours - per unit
                    "operating_hours_per_unit_limit": gen.get(
                        "operatingHoursPerUnitLimit"
                    ),
                    "operating_hours_per_unit_rolling_window": gen.get(
                        "operatingHoursPerUnitRollingWindow"
                    ),
                    # Operating hours - combined
                    "operating_hours_combined_limit": gen.get(
                        "operatingHoursCombinedLimit"
                    ),
                    "operating_hours_combined_group_ref": gen.get(
                        "operatingHoursCombinedGroupRef"
                    ),
                    "operating_hours_combined_rolling_window": gen.get(
                        "operatingHoursCombinedRollingWindow"
                    ),
                    ...
                    "recordkeeping_window_years": gen.get(
    ...
    def consolidate(self, output_path: Path = None) -> pd.DataFrame:
        ...
        for json_file in json_files:
            try:
                ...
                metadata = (
                    {
                        ...
                        "processing_time_sec": full_data.get(
                            "processing_time_sec"
                        ),
                        "completeness_score": full_data.get(
                            "completeness_score"
                        ),
                ...
                records = self.flatten_permit(
                    permit_data, source_file=json_file.name, metadata=metadata
                )
                all_records.extend(records)
                logger.debug(f"  ✓ Extracted {len(records)} generator records")

            except Exception as e:
                error_msg = f"Error processing {json_file.name}: {e!s}"
        ...
        if output_path:
            ...
            df.to_csv(
                output_path, index=False, encoding="utf-8", errors="replace"
            )
            logger.info("✓ Saved consolidated data to: %s", output_path)
    ...
    def generate_summary(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "total_capacity_kw": float(df["rated_capacity_kw"].sum())
            if "rated_capacity_kw" in df.columns
            else 0,
            "counties": int(df["facility_county"].nunique())
            if "facility_county" in df.columns
            else 0,
        }
    ...
    def save_styled_excel(
        self, df: pd.DataFrame, output_path: Path, highlight_warning_rows=True
    ):
        """
        ...
        df_excel = df.drop(
            columns=[col for col in internal_cols if col in df.columns],
            errors="ignore",
        )
        ...
        # Manual capacity retrieved
        manual_fill = PatternFill(
            start_color="DAF8D7", end_color="DAF8D7", fill_type="solid"
        )
        manual_fill_alt = PatternFill(
            start_color="C3F6CA", end_color="C3F6CA", fill_type="solid"
        )
        ...
        if "rated_capacity_bhp" in df_excel.columns:
            capacity_cols_map["rated_capacity_bhp"] = (
                df_excel.columns.get_loc("rated_capacity_bhp") + 1
            )
        if "rated_capacity_hp" in df_excel.columns:
            capacity_cols_map["rated_capacity_hp"] = (
                df_excel.columns.get_loc("rated_capacity_hp") + 1
            )
        if "rated_capacity_kw" in df_excel.columns:
            capacity_cols_map["rated_capacity_kw"] = (
                df_excel.columns.get_loc("rated_capacity_kw") + 1
            )
        if "rated_capacity_mmbtu_per_hr" in df_excel.columns:
            capacity_cols_map["rated_capacity_mmbtu_per_hr"] = (
                df_excel.columns.get_loc("rated_capacity_mmbtu_per_hr") + 1
            )
        ...
        for row_num in range(2, len(df) + 2):
            ...
            if highlight_warning_rows:
                # Get data quality severity for this row
                severity = df.iloc[df_row_idx].get(
                    "data_quality_severity", "ok"
                )
                ...
                if severity == "critical":
                    base_row_fill = (
                        missing_fill_alt if use_alt_color else missing_fill
                    )
                elif severity == "warning":
                    base_row_fill = (
                        warning_fill_alt if use_alt_color else warning_fill
                    )
                else:  # 'ok' - use standard alternating colors
                    base_row_fill = (
                        alt_row_dark if use_alt_color else alt_row_light
                    )
            ...
            for col_num in range(1, len(df_excel.columns) + 1):
                ...
                is_calculated = False
                if (
                    col_name == "rated_capacity_bhp"
                    and "calculated_bhp" in df.columns
                ):
                    is_calculated = df.iloc[df_row_idx]["calculated_bhp"]
                elif (
                    col_name == "rated_capacity_hp"
                    and "calculated_hp" in df.columns
                ):
                    is_calculated = df.iloc[df_row_idx]["calculated_hp"]
                elif (
                    col_name == "rated_capacity_kw"
                    and "calculated_kw" in df.columns
                ):
                    is_calculated = df.iloc[df_row_idx]["calculated_kw"]
                elif (
                    col_name == "rated_capacity_mmbtu_per_hr"
                    and "calculated_mmbtu_per_hr" in df.columns
                ):
                    is_calculated = df.iloc[df_row_idx][
                        "calculated_mmbtu_per_hr"
                    ]
                ...
                if is_calculated:
                    cell.fill = (
                        calculated_fill_alt
                        if use_alt_color
                        else calculated_fill
                    )
                else:
                    cell.fill = base_row_fill

                # Check if this specific capacity cell was calculated
                is_manual_retrieved = False
                if (
                    col_name
                    in {
                        "rated_capacity_bhp",
                    ...
                    and "manual_capacity_lookup" in df.columns
                ):
                    is_manual_retrieved = df.iloc[df_row_idx][
                        "manual_capacity_lookup"
                    ]

                # Apply cell-specific fill: blue for calculated, otherwise use row fill
                if is_manual_retrieved:
                    cell.fill = (
                        manual_fill_alt if use_alt_color else manual_fill
                    )
        ...
        for col_num, column in enumerate(df_excel.columns, 1):
            ...
            if (
                ...
                x in column_name
                for x in ["limit", "capacity", "hours", "percent", "pct"]
            ):
                width = 15
            else:
                width = 18
        ...
        logger.info(
            "  🔵 Blue in capacity cells = Calculated from other units"
        )
    ...
    def _create_instructions_sheet(self, writer):
        ...
        instructions_data.extend(
            [
                ...
                [
                    ...
                    "Check source permit - may need manual data entry",
                    "",
                ],
                [
                    "⚡ Orange ROW",
                    "Unusual capacity (verify)",
                    "Large/small capacity or high count - confirm it's correct",
                    "",
                ],
                [
                    "🔵 Blue in capacity CELL",
                    "We calculated this value",
                    "That specific capacity cell was calculated from another unit",
                    "",
                ],
                [
                    "⚪ Gray/White ROW",
                    "Complete & normal capacity",
                    "No action needed",
                    "",
                ],
                ["", "", "", ""],
        ...
        instructions_data.extend(
            [
                ...
                [
                    "HP/BHP → kW",
                    "kW = HP x 0.746 x Eff_gen (90%)",
                    "Standard electrical conversion",
                    "",
                ],
                [
                    "kW → HP/BHP",
                    "HP = kW ÷ 0.746 ÷ Eff_gen (90%)",
                    "Reverse conversion",
                    "",
                ],
                [
                    "MMBtu/hr → kW",
                    "kW = (MMBtu/hr x 0.35 x 1M) ÷ 3412",
                    "Thermal efficiency = 35%",
                    "",
                ],
                [
                    "kW → MMBtu/hr",
                    "MMBtu/hr = (kW × 3412) ÷ (Eff_fuel × 1,000,000)",
                    "Reverse thermal conversion",
                    "",
                ],
                ["", "", "", ""],
    ...
    def _create_summary_sheet(self, writer, df: pd.DataFrame):
        ...
        total_facilities = (
            df["facility_name"].nunique()
            if "facility_name" in df.columns
            else 0
        )
        total_permits = (
            df["permit_number"].nunique()
        ...
        summary_data.append(
            [
                "Any Capacity Field",
                missing_capacity,
                f"{missing_capacity / total_records * 100:.1f}%",
            ]
        )
        ...
        df_summary.to_excel(
            writer, sheet_name="Summary Statistics", index=False, header=False
        )
