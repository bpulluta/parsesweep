...
def create_file_tree(root_path: Path, title: str = None) -> Tree:
    ...
    tree = Tree(
        f"📁 [bold]{title or root_path.name}[/bold]",
        guide_style="dim"
    )
    
    def add_items(parent_tree: Tree, path: Path, max_depth: int = 2, current_depth: int = 0):
...
"""
    ...
    console.print()
    console.print(Panel(content, title="💰 Cost Estimation", border_style="cyan", padding=(1, 2)))
    console.print()
