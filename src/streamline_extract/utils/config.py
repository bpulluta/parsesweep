...
class Config:
    ...
    def _load_llm_config(self) -> dict:
        ...
        if 'AZURE_OPENAI_API_KEY' in env_vars and 'AZURE_OPENAI_MODEL' in env_vars:
            ...
            config['azure_api_version'] = env_vars.get('AZURE_OPENAI_API_VERSION', '2024-02-15-preview')
        elif 'ANTHROPIC_API_KEY' in env_vars:
            config['provider'] = 'anthropic'
            config['model'] = env_vars.get('ANTHROPIC_MODEL', 'claude-3.5-sonnet')
            ...
            config['api_key'] = env_vars.get('GEMINI_API_KEY') or env_vars.get('GOOGLE_API_KEY')
        ...
        if config['provider']:
            logger.info(f"Loaded LLM config: provider={config['provider']}, model={config['model']}")
        else:
