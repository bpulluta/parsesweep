            ...
            if len(parts) == 2:
                try:
                    ...
                    if "invalid literal" in str(e):
                        raise ValueError(f"Invalid page numbers in '{page_spec}'")
                    raise
...
def load_pages_csv(csv_path: Path) -> Dict[str, Optional[Tuple[int, int]]]:
    ...
    with open(csv_path, 'r') as f:
        ...
        # Validate headers
        if not all(h in reader.fieldnames for h in ['file_path', 'start_page', 'end_page']):
            raise ValueError(
        ...
        for row_num, row in enumerate(reader, start=2):  # Start at 2 (header is 1)
            ...
            if not start_str and not end_str:
                ...
                try:
                    ...
                    if start < 1 or end < 1:
                        raise ValueError(f"Page numbers must be >= 1 (row {row_num})")
                    if start > end:
                        raise ValueError(
                    ...
                    if "invalid literal" in str(e):
                        raise ValueError(f"Invalid page numbers in row {row_num}: {file_path}")
                    raise
            else:
                raise ValueError(
