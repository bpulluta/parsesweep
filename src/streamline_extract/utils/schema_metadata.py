...
class SchemaMetadata:
    ...
    def __init__(self, schema_path: Path, metadata_overrides: Optional[Dict[str, Any]] = None):
        """
        ...
        if metadata_overrides:
            self.metadata = self._deep_merge_dicts(self.metadata, metadata_overrides)
    ...
    def _collect_object_field_severity_hints(
        ...
        *,
        ...
        for field_name, field_schema in properties.items():
            ...
            severity = self._infer_schema_field_severity(field_name, field_schema)
            if severity:
    ...
    def _infer_schema_field_severity(
        ...
        if any(field_type in {"number", "integer"} for field_type in field_types):
            return "high"

        if any(token in normalized_name for token in HIGH_SEVERITY_SCHEMA_TOKENS):
            return "high"

        if field_schema.get("enum"):
            return "medium"

        if any(token in normalized_name for token in MEDIUM_SEVERITY_SCHEMA_TOKENS):
            return "medium"
