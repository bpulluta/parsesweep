...
def find_companion_qaqc_schema(production_schema_path: Path) -> Tuple[Optional[Path], Optional[dict]]:
    ...
    production_stem = production_schema_path.stem  # e.g., "geothermal_ordinance_schema"
    ...
    if production_stem.endswith("_schema"):
        ...
        if derived_path.exists():
            try:
                ...
                logger.info(f"Found companion QA/QC schema (by naming convention): {derived_path}")
                return derived_path, qaqc_schema
            except Exception as e:
                logger.warning(f"Error loading QA/QC schema {derived_path}: {e}")
    ...
    for qaqc_file in qaqc_dir.glob("*_qaqc.json"):
        try:
            ...
            companion_to = qaqc_schema.get("$metadata", {}).get("companion_to", "")
            # Check if companion_to matches our production schema
            if companion_to and (companion_to == production_name or 
                                 companion_to.endswith(f"/{production_name}") or
                                 production_name in companion_to):
                logger.info(f"Found companion QA/QC schema (by companion_to): {qaqc_file}")
