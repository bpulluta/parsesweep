...
class ExtractionCleaner:
    ...
    def load_extraction(self, filepath: Path) -> dict:
        ...
        try:
            ...
            self.errors.append(
                {"file": str(filepath), "error": str(e), "stage": "loading"}
            )
            return None
    ...
    def collect_all_files(self) -> dict[str, list[dict]]:
        ...
        for category_dir in self.extracted_dir.iterdir():
            ...
            for json_file in category_dir.glob("*.json"):
                ...
                entity_id = self.schema_metadata.extract_identifier_from_data(
                    data
                )
    ...
    def identify_entity_versions(
        ...
        for entity_id, files in entity_groups.items():
            if len(files) > 1:
                ...
                sorted_files = sorted(
                    files,
                    key=lambda x: (x["completeness"], x["item_count"]),
                    reverse=True,
                )
                ...
                version_reports.append(
                    {
                        ...
                        "files_to_keep": [
                            f["path"].name for f in files_to_keep
                        ]
                        if deduplicate
                        else None,
                        "versions": [
    ...
    def create_cleaned_dataset(
            ...
            for version_group in self.entity_versions:
                ...
                if version_group.get("files_to_keep"):
                    ...
                    if version_group.get("is_true_duplicate"):
                        ...
                        removed_count = version_group["version_count"] - len(
                            version_group["files_to_keep"]
                        )
        ...
        self.stats["files_with_items"] = sum(
            1
            for files in entity_groups.values()
            for f in files
            if f["item_count"] > 0
        )
    ...
    def generate_reports(self):
        ...
        with open(
            self.reports_dir / "consolidation_summary.json",
            "w",
            encoding="utf-8",
        ) as f:
