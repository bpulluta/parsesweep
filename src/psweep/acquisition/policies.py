"""Policy evaluation helpers for acquisition network access."""
...
class PolicyCheckResult:
    """Outcome of evaluating acquisition policy controls for a target URL."""
...
class AcquisitionPolicyEvaluator:
    """Evaluate robots.txt and manual ToS acknowledgement policy controls."""

    DEFAULT_USER_AGENT = "ParseSweep/2.0 (+acquisition)"
    ...
    def evaluate(
        ...
        *,
        ...
        message = f"robots.txt disallows acquisition for '{url}'."
        if normalized_robots_mode == "enforce":
            result.allowed = False
